import type { Plugin } from '@pdfme/common';
import type { Schema } from '@pdfme/common';
import imageSchema from './image.js';

/**
 * Signature plugin for @pdfme/schemas.
 *
 * Signatures are captured as PNG data URLs (e.g. via a canvas/signature-pad in the UI)
 * and rendered as images in the PDF. This plugin reuses the image plugin's pdf and ui
 * renderers so that any base64 PNG stored in `content` is embedded correctly.
 *
 * In the browser (playground / form UI) you can override the `ui` property with a
 * signature-pad implementation that writes the drawn signature back via
 * `onChange({ key: 'content', value: dataUrl })`.
 */

const DEFAULT_SIGNATURE_CONTENT =
  'data:image/png;base64,';

type SignatureSchema = Schema;

const signatureSchema: Plugin<SignatureSchema> = {
  // PDF rendering: signatures are stored as PNG data URLs, identical to images.
  pdf: imageSchema.pdf,

  // UI rendering: falls back to the image display/upload UI.
  // In a browser environment (playground, form), override this property with
  // a SignaturePad-based implementation that draws on a canvas and calls
  // onChange({ key: 'content', value: dataUrl }) on stroke end.
  ui: imageSchema.ui,

  propPanel: {
    schema: {},
    defaultSchema: {
      name: '',
      type: 'signature',
      content: DEFAULT_SIGNATURE_CONTENT,
      position: { x: 0, y: 0 },
      width: 62.5,
      height: 37.5,
    },
  },

  icon: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>',
};

export default signatureSchema;
