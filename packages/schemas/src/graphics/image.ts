...
import type { Plugin, PropPanelWidgetProps } from '@pdfme/common';
...
const imageNameSelectorWidget = (props: PropPanelWidgetProps) => {
  const { rootElement, changeSchemas, activeSchema, options } = props;
  const imageVariables = (options as any).variables?.imageVariables ?? [];

  console.log('[imageNameSelectorWidget] options.variables:', (options as any).variables)
  console.log('[imageNameSelectorWidget] imageVariables:', imageVariables)
  ...
  lbl.textContent = 'Field Name (Image Variable)';
  ...
  blank.value = '';
  blank.textContent = '— select image variable —';
  ...
  propPanel: {
    schema: ({ activeSchema, options }) => {
      ...
      return {
        name: {
          type: 'void',
          widget: 'imageNameSelectorWidget',
          bind: false,
          span: 24,
        },
      };
    },
    widgets: { imageNameSelectorWidget },
    defaultSchema: {
      name: '',
      type: 'image',
